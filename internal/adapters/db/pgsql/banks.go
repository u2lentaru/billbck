...
func (est *BankStorage) Add(ctx context.Context, a models.Bank) (int, error) {
	...
	err := dbpool.QueryRow(ctx, "SELECT func_banks_add($1,$2,$3);", a.BankName, a.BankDescr, a.Mfo).Scan(&ai)
...
func (est *BankStorage) Upd(ctx context.Context, u models.Bank) (int, error) {
	...
	err := dbpool.QueryRow(ctx, "SELECT func_banks_upd($1,$2,$3,$4);", u.Id, u.BankName, u.BankDescr, u.Mfo).Scan(&ui)
...
func (est *BankStorage) Del(ctx context.Context, d []int) ([]int, error) {
	...
	i := 0
	for _, id := range d {
		err := dbpool.QueryRow(ctx, "SELECT func_banks_del($1);", id).Scan(&i)
		res = append(res, i)
...
func (est *BankStorage) GetOne(ctx context.Context, i int) (models.Bank_count, error) {
	...
	err := dbpool.QueryRow(ctx, "SELECT * from func_bank_get($1);", i).Scan(&g.Id, &g.BankName, &g.BankDescr, &g.Mfo)
	...
	out_count := models.Bank_count{Values: out_arr, Count: 1, Auth: models.Auth{}}
	return out_count, nil
}
