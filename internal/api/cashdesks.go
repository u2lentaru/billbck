...
func (s *APG) HandleCashdesks(w http.ResponseWriter, r *http.Request) {
	...
	gsc := 0
	err := s.Dbpool.QueryRow(ctx, "SELECT * from func_cashdesks_cnt($1);", gs1).Scan(&gsc)
	...
	rows, err := s.Dbpool.Query(ctx, "SELECT * from func_cashdesks_get($1,$2,$3,$4,$5);", pg, pgs, gs1, ord, dsc)
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
	...
	out_count, err := json.Marshal(models.Cashdesk_count{Values: out_arr, Count: gsc, Auth: auth})
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
...
func (s *APG) HandleAddCashdesk(w http.ResponseWriter, r *http.Request) {
	...
	ai := 0
	err = s.Dbpool.QueryRow(context.Background(), "SELECT func_cashdesks_add($1);", a.CashdeskName).Scan(&ai)
...
func (s *APG) HandleUpdCashdesk(w http.ResponseWriter, r *http.Request) {
	...
	ui := 0
	err = s.Dbpool.QueryRow(context.Background(), "SELECT func_cashdesks_upd($1,$2);", u.Id, u.CashdeskName).Scan(&ui)
...
func (s *APG) HandleDelCashdesk(w http.ResponseWriter, r *http.Request) {
	...
	for _, id := range d.Ids {
		err = s.Dbpool.QueryRow(context.Background(), "SELECT func_cashdesks_del($1);", id).Scan(&i)
		res = append(res, i)
...
func (s *APG) HandleGetCashdesk(w http.ResponseWriter, r *http.Request) {
	...
	err := s.Dbpool.QueryRow(context.Background(), "SELECT * from func_cashdesk_get($1);", i).Scan(&g.Id, &g.CashdeskName)
	...
	out_count, err := json.Marshal(models.Cashdesk_count{Values: out_arr, Count: 1, Auth: auth})
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
