...
func (s *APG) HandleEso(w http.ResponseWriter, r *http.Request) {
	...
	gsc := 0
	err := s.Dbpool.QueryRow(ctx, "SELECT * from func_eso_cnt($1);", gs1).Scan(&gsc)
	...
	rows, err := s.Dbpool.Query(ctx, "SELECT * from func_eso_get($1,$2,$3,$4,$5);", pg, pgs, gs1, ord, dsc)
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
	...
	out_count, err := json.Marshal(models.Eso_count{Values: out_arr, Count: gsc, Auth: auth})
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
...
func (s *APG) HandleAddEso(w http.ResponseWriter, r *http.Request) {
	...
	ai := 0
	err = s.Dbpool.QueryRow(context.Background(), "SELECT func_eso_add($1);", a.EsoName).Scan(&ai)
...
func (s *APG) HandleUpdEso(w http.ResponseWriter, r *http.Request) {
	...
	ui := 0
	err = s.Dbpool.QueryRow(context.Background(), "SELECT func_eso_upd($1,$2);", u.Id, u.EsoName).Scan(&ui)
...
func (s *APG) HandleDelEso(w http.ResponseWriter, r *http.Request) {
	...
	for _, id := range d.Ids {
		err = s.Dbpool.QueryRow(context.Background(), "SELECT func_eso_del($1);", id).Scan(&i)
		res = append(res, i)
...
func (s *APG) HandleGetEso(w http.ResponseWriter, r *http.Request) {
	...
	err := s.Dbpool.QueryRow(context.Background(), "SELECT * from func_eso_getbyid($1);", i).Scan(&g.Id, &g.EsoName)
	...
	out_count, err := json.Marshal(models.Eso_count{Values: out_arr, Count: 1, Auth: auth})
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
