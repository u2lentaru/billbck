...
func (s *APG) HandleSectors(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.Dbpool.Query(ctx, "SELECT * from func_sectors_get($1,$2,$3,$4,$5);", pg, pgs, gs1, ord, dsc)
	if err != nil {
	...
	gsc := 0
	err = s.Dbpool.QueryRow(ctx, "SELECT * from func_sectors_cnt($1);", gs1).Scan(&gsc)
	...
	out_count, err := json.Marshal(models.Sector_count{Values: out_arr, Count: gsc, Auth: auth})
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
...
func (s *APG) HandleAddSector(w http.ResponseWriter, r *http.Request) {
	...
	ai := 0
	err = s.Dbpool.QueryRow(context.Background(), "SELECT func_sectors_add($1);", a.SectorName).Scan(&ai)
...
func (s *APG) HandleUpdSector(w http.ResponseWriter, r *http.Request) {
	...
	ui := 0
	err = s.Dbpool.QueryRow(context.Background(), "SELECT func_sectors_upd($1,$2);", u.Id, u.SectorName).Scan(&ui)
...
func (s *APG) HandleDelSector(w http.ResponseWriter, r *http.Request) {
	...
	for _, id := range d.Ids {
		err = s.Dbpool.QueryRow(context.Background(), "SELECT func_sectors_del($1);", id).Scan(&i)
		res = append(res, i)
...
func (s *APG) HandleGetSector(w http.ResponseWriter, r *http.Request) {
	...
	err := s.Dbpool.QueryRow(context.Background(), "SELECT * from func_sector_get($1);", i).Scan(&g.Id, &g.SectorName)

	if err != nil && err != pgx.ErrNoRows {
		log.Println("Failed execute from func_sector_get: ", err)
	}
	...
	out_count, err := json.Marshal(models.Sector_count{Values: out_arr, Count: 1, Auth: auth})
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
